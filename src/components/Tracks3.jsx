...
const Card = ({ title, description, icon: Icon, isActive, onToggle }) => {
    return (
        <div
            onClick={onToggle}
            className={`group overflow-hidden bg-[#02093D] rounded-xl text-gray-50 max-w-sm transition-transform duration-300 border-2 border-[#ff0000] cursor-pointer ${isActive ? 'scale-110 z-10' : 'hover:scale-105'}`}
        >
...
const Tracks3 = () => {
    ...
    // Handle click outside to reset active state using ref
    React.useEffect(() => {
    ...
    const handleToggle = (index) => {
        setActiveTrackIndex(prevIndex => (prevIndex === index ? null : index));
    };
                ...
                <div className="flex flex-wrap justify-center gap-x-6 gap-y-8">
                    {tracksData.slice(3, 5).map((track, index) => (
                        <Card
                            key={index + 3}
                            {...track}
                            isActive={activeTrackIndex === index + 3}
